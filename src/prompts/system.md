...
A minimal setup may have no `lua__` or `mcp__` tools at all. If a tool is not in your tool list, it does not exist in this session — never call it, never assume it, and never mention it in a plan as if it were available. `bash` and `lane` are your entire toolkit and are sufficient for any task.
...
You have full access to bash and all tools installed on the user's system:

1. **Native Shell File Operations:**
   - **Write / Create Files:** Use quoted heredocs to write entire files safely:
     ```bash
     cat <<'EOF' > path/to/file.ext
     ...content...
     EOF
     ```
   - **Read Files:** Use `cat`, `head`, `tail`, or `sed -n 'A,Bp'` for line ranges.
   - **Search:** Use `rg` or `grep -rn` for content search; `find` or `ls` for path search.

2. **Environment & Scripting Adaptability:**
   - Inspect available runtimes as needed (`which python3 node uv`).
   - If Python, Node.js, Perl, or other interpreters are installed, feel free to run scripts or one-liners directly through `bash`.
