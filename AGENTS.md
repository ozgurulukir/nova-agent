...
- **Modular Protocol & Execution Decomposition:**
  - **`responses_request.zig` & `responses_events.zig`:** Wire request payload serialization and SSE delta/event decoding are cleanly separated from `responses_core.zig` (`INV-RESP-1`). All JSON schema formatting, dialect reasoning clipping (`wireEffortLabel`), and prompt-cache suppressions (`disable_prompt_cache`) live in `responses_request.zig`. Event accumulation, UTF-8 delta handling, synthetic call ID generation, and usage clamping live in `responses_events.zig`.
  - **`executor_safety.zig` & `executor_validation.zig`:** Tool execution boundaries in `src/executor.zig` delegate validation and shell safety (`INV-EXEC-1`). Shell command classification, chained destructive command handling (`rm -rf /; ...`), and interactive observer confirmations live in `executor_safety.zig`. Multi-channel schema resolution (builtin/MCP/plugin) and argument type coercion/validation live in `executor_validation.zig`.
  - **TUI Presentation Decoupling (`INV-WIDGET-1`):** Leaf widgets under `src/tui/widgets/` (`message.zig`, `transcript.zig`) must not reference `App` directly. Pass scalar configurations (`has_model_configured: bool`) or isolated view models.
  - **Leaf Library Isolation (`INV-LEAF-1`):** Modules under `lib/` are pure leaves with zero imports from `src/` and compile independently.

- **Parallel lanes & worktree architecture.** Design spec: `_plan/plan-model-driven-lanes-2026-08-03.md` and worktree hardening plans (`_plan/plan-worktree-*.md`). Key files: `src/tools/lane.zig` (tool side, workspace borrow), `src/tools/lane_bridge.zig` (request/response bridge), `src/tui/lane_lifecycle.zig` (all lane ops + teardown + async worktree job), `src/tui/lifecycle.zig` (`handleTick` order: `drainAgentEvents` → `serviceLaneBridge` → `drainLaneNaming` → `deliverPendingLaneCompletions`; `createParallelLane` user flow). Limits: max 4 threads (driver + 3 lanes); global worktrees at `<home>/.config/nova/worktrees/<id>` resolved via `vcs.globalWorktreesDir`, branch `nova/<id>`; `worker_stall_ms = 180s`.
- **Worktree Invariants & Safety:**
  - **Path Normalization:** Always use `lanes_util.pathsEqual` (zero-allocation, slash-agnostic, case-folded on Windows) for comparing workspace borrow paths and worktree directories. Never use byte-level `std.mem.eql`.
  ...
  - **PowerShell Root Containment:** `pwsh.zig` enforces root containment with `[System.IO.Path]::GetFullPath`, `pathsEqual`, `$root.TrimEnd('\', '/')`, and blocks directory escaping (`Push-Location`/`Pop-Location`).
...
- **Mention/image ingestion caps.** `at_mention.zig` caps a single file mention at 64KB (`per_file_mention_max_bytes`), an aggregate of 256KB per turn (`turn_mention_aggregate_max_bytes`), and 4 images per message (`max_images_per_message`). Oversized files are read as a REAL head+tail sandwich (first half + last half of the budget, joined by `common.elideMiddle`) so a plan's load-bearing conclusion at the tail survives, with a `[file truncated: {n} bytes — re-read with read_file if you need the rest]` notice; over-cap images are skipped, not truncated. The elision format is the shared `common.elideMiddle` SSOT (also used by `pruneToolText` for tool-result truncation).
